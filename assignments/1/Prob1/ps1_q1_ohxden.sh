...
do
    file=OHXDEN_$col2.XPT
    if [ ! -f $file ]; then
    url=https://wwwn.cdc.gov/Nchs/Nhanes/$col1/OHXDEN_$col2.XPT
    wget $url
    fi
...
while read col1 col2
do
    file=short_$col2.csv
    if [ ! -f $file ]; then
    zsh cutnames.sh OHXDEN_$col2.csv  "SEQN|OHDDESTS|OHX[0-9]*TC|OHX[0-9]*CTC" | tail -n+2 > $file
    fi
done < nhanes_files.txt

## appends all years into a single file nhanes_ohxden.csv
#total_file="nhanes_ohxden.csv"

## first check whether the columns match
header_file="header_OHXDEN.txt"
...
file_name="nhanes_ohxden.csv"

if [ ! -f "$file_name" ]; then
    < short_G.csv head -n1  > "$file_name"
    while read col1 col2
    do
        file=short_$col2.csv
